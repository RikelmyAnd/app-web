...
export const VendaProcesso = () => {
    ...
    const carregarDadosIniciais = async () => {
        ...
        if (pedidoId) {
            ...
            const sessaoAtual = await sessoesService.findById(pedido.sessaoId); // Assumindo que seu service tem findById
            setSessao(sessaoAtual);
            setIngressosSelecionados(pedido.ingressos);
            setLanchesSelecionados(pedido.lanches);
        } else if (sessaoId) {
            // Modo Nova Compra
            // Nota: Se o service não tiver findById, faça um findAll e .find()
            const sessoes = await sessoesService.findAll();
            const encontrada = sessoes.find(s => s.id === sessaoId);
            if (encontrada) setSessao(encontrada);
        }
    };
    ...
    const adicionarIngresso = () => {
        ...
        if (ingressosSelecionados.length >= 100) { // Exemplo estático, ideal pegar do 'sessao.salaId'
            alert("Capacidade máxima atingida (simulação).");
            return;
        }
